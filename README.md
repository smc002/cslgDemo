...
主工程 `E:/cslgDemo`，浏览器交付、单机本地存档。当前版本 0.7：明亮卡通竖屏界面、可交互 2D 内城与六阶段建筑、五位英雄序列帧、修复猎场僵尸串图。保留物资搜刮 → 禁区猎场 → 英雄成长与招募 → 内城补给闭环，普通猎场射速 4 发/秒。

双击 **Start-Demo.cmd**。本机：http://localhost:3010/#merged 。同一局域网：http://10.100.102.227:3010/#merged （电脑服务需运行，IP 变化时更新地址）。未修改旧 8080 原型服务。

## 试玩顺序

1. 开局三人：坦克、医疗兵、无人机输出。可拖动或点击换位，点击“开始搜刮”。支持暂停和 1/2/4 倍速。
...
5. 第二关开放第二路、三名援军和内城；第四关开放第三路与另三名援军。内城免费工坊加 100 建材足以建造首座黑金弹工厂。
6. 切换到猎场、英雄或内城时，搜刮继续，小窗显示实时进度。关闭/挂起浏览器不模拟离线战斗；内城依据时间戳累积，最多储存 20 批。

设置中可以领取有限应急补给，以及二次确认重置存档。新循环使用 `mori.demo.campaign.v2`，旧版 v1 存档保留并备份，未将旧原型无限波次/赠券进度直接换算为本版本关卡。
...
这是闭环接入版，数值为集中声明的试玩参数。引导为电台提示与分阶段解锁；完整章节剧情、全角色动画、成套音效、真机性能调优和长线数值仍属后续两个月工作。
...
详见 [游戏设计](docs/游戏设计.md)、[技术架构](docs/技术架构.md)、[项目进展](docs/项目进展.md)。未修改 E:/mori、Unity 正式工程；没有删除 Git 分支或推送提交。
